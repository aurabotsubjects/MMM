...
async function turnOffRelieverAccess() {
    ...
    if (error) { console.error('disable_reliever_access RPC error:', error); showToast('Could not turn off reliever access: ' + (error.message || 'unknown error')); return; }
    if (!data || data.error) { console.error('disable_reliever_access returned an error:', data); showToast('Could not turn off reliever access'); return; }
    closeRelieverModal();
